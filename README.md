...
// Kiểm tra sinh viên chưa có phiếu đang active
const existingRegistration = await prisma.registration.findFirst({
  where: {
    studentId: student.id,
    status: { in: ['CHO_XAC_NHAN', 'DA_XAC_NHAN', 'DA_THANH_TOAN'] }
  }
})
if (existingRegistration) {
  throw new Error("Bạn đã có phiếu đăng ký đang hoạt động")
}
\`\`\`
...
**Authorization:**
- Chỉ sinh viên sở hữu phiếu mới được hủy
- Chỉ hủy được khi trạng thái là "CHO_XAC_NHAN"
...
await prisma.$transaction([
  ...
  prisma.room.update({
    ...
    data: { soGiuongDaDuocDat: { decrement: 1 } }
  })
])
\`\`\`
...
const validTransitions = {
  ...
  'DA_THANH_TOAN': [], // Không thể thay đổi
...
File `middleware.ts` bảo vệ routes dựa trên vai trò:
...
Mỗi Server Action kiểm tra quyền trước khi thực thi:
...
\`\`\`typescript
// Đăng nhập
login(credentials: LoginInput): Promise<{ success: boolean }>
...
### Registration Actions (`actions/registration.ts`)

\`\`\`typescript
// Tạo phiếu đăng ký mới
createRegistration(data: CreateRegistrationInput): Promise<Registration>

// Lấy phiếu đăng ký của sinh viên
...
getAllRegistrations(filters?: FilterOptions): Promise<Registration[]>
...
### Room Actions (`actions/room.ts`)

\`\`\`typescript
// Lấy danh sách phòng có sẵn
getAvailableRooms(filters?: RoomFilters): Promise<Room[]>
...
\`\`\`bash
# Reset database (CHÚ Ý: Xóa toàn bộ dữ liệu)
...
Nếu gặp lỗi "Session secret must be at least 32 characters":
- Tạo SESSION_SECRET mới với ít nhất 32 ký tự
